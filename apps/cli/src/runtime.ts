...
import { GitRepository as Repository } from "@orbit/git-store";
import { inspectWorkspace } from "@orbit/project";
...
import { buildContext, workspaceWarnings } from "@orbit/handoff";
import { PublishWorker } from "./publish.js";
import type {
  Project,
  Session,
  UniversalEvent,
...
export async function continueContext(
  ...
  if (handoff.truncated)
    console.error("Orbit: context is limited to the original goal, latest request, and recent history that fits this agent's context budget.");
...
export async function runAgent(
  ...
  async function tick() {
    ...
    capturing = (async () => {
      ...
      const { readRecords } = await import("@orbit/adapter-kit");
      ...
      if (kept.length) {
        w = {
          ...w,
          title: w.title === "Untitled workstream" && objective?.payload.type === "user_message"
  ...
  try {
    ...
    if (session.status === "ended") {
      try {
        const intelligence = await import("@orbit/api");
        const settings = await intelligence.getIntelligenceSettings(repo);
        if (settings.autoSummarize && settings.provider) {
          console.error("Orbit: generating a private session summary with " + settings.provider + ".");
          await intelligence.summarizeSession(repo, pid, session.id);
        }
      } catch (error) {
        console.error("Orbit summary: " + (error instanceof Error ? error.message : String(error)));
      }
    }
    stopped = true;
