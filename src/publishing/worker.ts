...
import { GitRepository, gitText } from "../storage/git/index.js";

async function putChanged(
  repo: GitRepository,
  pid: string,
  kind: "project" | "workstream" | "session" | "event" | "summary",
  value: any,
) {
  const prior = await repo.get(pid, kind, value.id);
  if (JSON.stringify(prior) !== JSON.stringify(value))
    await repo.put(pid, kind, value);
}
...
export async function buildPublishProjection(source: GitRepository) {
  ...
  const project =
    (await source
      .get<Project>(
        (await source.state("project:id")) ?? "",
        "project",
        (await source.state("project:id")) ?? "",
      )
      .catch(() => null)) ??
    (
      await source.db.query<{ data: string }>(
        "SELECT data FROM orbit_entities WHERE kind=$1 LIMIT 1",
        ["project"],
      )
    ).map((row) => JSON.parse(row.data) as Project)[0];
  if (!project) {
  ...
  const selected = new Set<string>(
    JSON.parse((await source.state("publish:selected:sessions")) ?? "[]"),
  );
  await putChanged(target, pid, "project", project);
  const sourceSessions = (await source.list<Session>(pid, "session")).filter(
  ...
  for (const session of await target.list<Session>(pid, "session"))
    if (!keepSessions.has(session.id))
      await target.remove(pid, "session", session.id);
  const workstreamIds = new Set(
    sourceSessions.map((session) => session.workstreamId),
  );
  ...
  for (const workstream of await target.list<Workstream>(pid, "workstream"))
    if (!workstreamIds.has(workstream.id))
      await target.remove(pid, "workstream", workstream.id);
...
export class PublishWorker {
  ...
  async flush(signal?: AbortSignal, force = false) {
    ...
    const target =
      (await this.repo.state("publish:mode")) === "selected-v1"
        ? await buildPublishProjection(this.repo)
        : this.repo;
    try {
      await target.push(
        "origin",
        await gitCredentials(target),
        signal ? 3000 : 30000,
      );
    } finally {
