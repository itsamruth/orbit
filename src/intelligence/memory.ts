  ...
  while (true) {
    ...
    for (const record of batch.records) {
      ...
      if (timestamp && Number.isFinite(Date.parse(timestamp)))
        startedAt =
          startedAt === null ? timestamp : [startedAt, timestamp].sort()[0]!;
      branch ??=
...
export async function listImportCandidates(
  ...
  return (await discover(repo, projectId, projectRoot, only)).map(
    (x) => x.candidate,
  );
}
...
export async function importHistoricalSessions(
  ...
  if (selected.some((x) => x.candidate.importState === "conflict"))
    throw new Error(
      "A native transcript changed before its saved offset; import was stopped",
    );
  ...
  for (const item of selected) {
    ...
    let workstream = await repo.get<Workstream>(
      projectId,
      "workstream",
      workstreamId,
    );
    ...
    const events = await repo.list<UniversalEvent>(
      projectId,
      "event",
      sessionId,
    );
  ...
  if (summarize)
    for (const session of imported)
      await summarizeSession(repo, projectId, session.id);
...
export async function getIntelligenceSettings(
  ...
  if (!raw)
    return { provider: null, autoSummarize: false, maxInputBytes: 48000 };
  ...
  try {
    ...
    if (!supported)
      return {
        ...
        message: "Update this agent CLI to use Orbit intelligence",
      };
    try {
      await exec(
        id,
      ...
      return {
        ...
        version: version.trim(),
        message: "Sign in with the agent CLI before enabling summaries",
      };
    }
    return {
      id,
      status: "ready",
      version: version.trim(),
      message: "Installed, authenticated, and compatible",
    };
  } catch (error) {
...
async function summarizeSessionNow(
  ...
  if (!provider)
    throw new Error("Choose a project intelligence provider first");
  const sourceHash = hash(JSON.stringify(events));
  const summaryId = stableId("summary", projectId, sessionId);
  const existing = await repo.get<SessionSummary>(
    projectId,
    "summary",
    summaryId,
  );
  ...
  try {
    ...
    const evidence = (value: unknown) =>
      (Array.isArray(value) ? value : []).flatMap((item: any) => {
        const evidenceEventIds = strings(item?.evidenceEventIds).filter((id) =>
          allowed.has(id),
        );
    ...
    const tasks = (Array.isArray(raw.tasks) ? raw.tasks : []).flatMap(
      (item: any) => {
        const evidenceEventIds = strings(item?.evidenceEventIds).filter((id) =>
          allowed.has(id),
        );
    ...
    await repo.setState(
      "summary:job:" + sessionId,
      JSON.stringify({
        status: auth ? "waiting_for_provider" : "failed",
        provider,
        message,
      }),
    );
...
export async function summarizeSession(
  ...
  const prior =
    summaryQueues.get(projectId) ??
    Promise.resolve(undefined as unknown as SessionSummary);
  const job = prior
    .catch(() => undefined as unknown as SessionSummary)
    .then(() =>
