...
export async function ensureSearchIndex(
  ...
  await repo.db.transaction(async (db) => {
    ...
    for (const summary of await repo.list<SessionSummary>(
      projectId,
      "summary",
    )) {
...
export async function searchIndex(
  ...
  if (!query || query.length > 200)
    throw new Error("Enter a search query up to 200 characters");
  ...
  let sql =
    "SELECT id,type,session_id,workstream_id,agent,occurred_at,title,body,bm25(orbit_fts) AS rank FROM orbit_fts WHERE orbit_fts MATCH $1";
  const filter = (
    column: string,
  ...
  sql +=
    " ORDER BY rank,occurred_at DESC LIMIT $" +
    (values.length - 1) +
    " OFFSET $" +
    values.length;
