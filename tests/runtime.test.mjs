...
import { SyncWorker } from "../packages/sync/dist/index.js";
import {
  PolicyFilter,
  defaultPolicy,
  redact,
} from "../packages/security/dist/index.js";
import { buildContext } from "../packages/handoff/dist/index.js";
import {
  readRecords,
  normalizeBatch,
} from "../packages/adapter-kit/dist/index.js";
import {
  codexAdapter,
  normalizeCodex,
} from "../packages/adapter-codex/dist/index.js";
import { normalizeClaude } from "../packages/adapter-claude/dist/index.js";
import { buildServer } from "../apps/api/dist/legacy-server.js";
...
async function server(t, db = new SqliteDatabase(":memory:")) {
  const app = await buildServer(db, { origin: "http://localhost:5173" });
  t.after(async () => {
    await app.close();
    await db.close();
  });
  ...
  const send = (method, url, payload, user = "owner") =>
    app.inject({
      method,
      url,
  ...
  await send("POST", "/api/v1/projects", { id: "prj_test", name: "test" });
  return { app, db, send };
}
...
test("API authenticates and isolates projects, validates uploads", async (t) => {
  const { app, send } = await server(t);
  assert.equal(
    (await app.inject({ method: "GET", url: "/api/v1/projects" })).statusCode,
    401,
  );
  assert.equal(
    (await send("GET", "/api/v1/projects/prj_test", undefined, "other"))
      .statusCode,
    404,
  );
  assert.equal(
    (
      await send("POST", "/api/v1/projects/prj_test/sync", {
        changes: [{}],
        cursor: null,
      })
    ).statusCode,
  ...
  assert.equal(
    (await send("PATCH", "/api/v1/projects/prj_test", { name: "" })).statusCode,
    400,
  );
});
test("API synchronization retries preserve one event and monotonically paginated history", async (t) => {
  const { send } = await server(t);
  const changes = [
    op("workstream", work()),
    op("session", session()),
    ...Array.from({ length: 55 }, (_, i) => op("event", event(i))),
  ];
  const first = await send("POST", "/api/v1/projects/prj_test/sync", {
    changes,
    cursor: null,
  });
  assert.equal(first.statusCode, 200, first.body);
  const retry = await send("POST", "/api/v1/projects/prj_test/sync", {
    changes,
    cursor: null,
  });
  assert.equal(retry.statusCode, 200, retry.body);
  const p1 = (
    await send("GET", "/api/v1/projects/prj_test/sessions/sess_test/events")
  ).json();
  assert.equal(p1.items.length, 50);
  const p2 = (
    await send(
      "GET",
      "/api/v1/projects/prj_test/sessions/sess_test/events?cursor=" +
        p1.nextCursor,
    )
  ).json();
  assert.equal(p2.items.length, 5);
  assert.equal(p2.nextCursor, null);
  assert.equal(p2.items[0].sequence, 50);
  const conflict = op("event", {
    ...event(),
    payload: { type: "user_message", text: "different" },
  });
  assert.equal(
    (
      await send("POST", "/api/v1/projects/prj_test/sync", {
...
test("offline history uploads, downloads on a second device, and deletion cannot resurrect", async (t) => {
  const { send } = await server(t);
  const a = await local(t);
  await a.repo.put("prj_test", "workstream", work());
  await a.repo.put("prj_test", "session", session());
  await a.repo.put("prj_test", "event", event());
  const transport = {
    async exchange(pid, changes, cursor) {
      const res = await send("POST", "/api/v1/projects/" + pid + "/sync", {
        changes,
        cursor,
      });
  ...
  const wa = new SyncWorker(a.repo, transport, "prj_test");
  await wa.flush();
  assert.equal((await a.repo.pending("prj_test")).length, 0);
  const bdb = new SqliteDatabase(":memory:");
  t.after(() => bdb.close());
  const b = new Repository(bdb);
  const wb = new SyncWorker(b, transport, "prj_test");
  await wb.flush();
  assert.equal((await b.list("prj_test", "event")).length, 1);
  await b.put("prj_test", "event", event(1));
  await a.repo.remove("prj_test", "workstream", "work_test");
  await wa.flush();
  await wb.flush();
  assert.equal((await b.list("prj_test", "event")).length, 0);
  assert.equal((await b.pending("prj_test")).length, 0);
});
test("local project deletion is still uploaded after the project record is removed", async (t) => {
  const { send } = await server(t);
  const { repo } = await local(t);
  const transport = {
    async exchange(pid, changes, cursor) {
  ...
  const worker = new SyncWorker(repo, transport, "prj_test");
  await worker.flush();
  await repo.remove("prj_test", "project", "prj_test");
  await worker.flush();
  assert.equal(
    (await send("GET", "/api/v1/projects/prj_test")).statusCode,
    410,
  );
  assert.equal((await repo.pending("prj_test")).length, 0);
});
test("revoked device tokens immediately lose access", async (t) => {
  const { send } = await server(t);
  assert.equal(
    (await send("DELETE", "/api/v1/devices/dev_owner")).statusCode,
    200,
  );
  assert.equal((await send("GET", "/api/v1/me")).statusCode, 401);
});
test("browser mutations require the configured Origin", async (t) => {
  const { app, db } = await server(t);
  await db.query("UPDATE orbit_credentials SET kind=$1 WHERE user_id=$2", [
    "web",
    "owner",
  ]);
  const res = await app.inject({
    method: "PATCH",
    url: "/api/v1/projects/prj_test",
    headers: {
      cookie: "orbit_session=owner_token",
...
test("device codes expire, require approval, and are consumed once", async (t) => {
  const { app, send } = await server(t);
  const started = (
    await app.inject({
      method: "POST",
      url: "/api/v1/auth/device/start",
      payload: { name: "Test laptop" },
    })
  ).json();
  const token = () =>
    app.inject({
  ...
  assert.equal((await token()).statusCode, 202);
  assert.equal(
    (await send("POST", "/api/v1/auth/device/approve", { code: started.code }))
      .statusCode,
    200,
  );
  const approved = await token();
  assert.equal(approved.statusCode, 200);
  assert.ok(approved.json().token);
  assert.equal((await token()).statusCode, 400);
});

test("deletion erases event bodies from the cloud change log and local outbox", async (t) => {
  const { send, db } = await server(t);
  const changes = [
    op("workstream", work()),
    op("session", session()),
    op(
      "event",
      event(0, { type: "user_message", text: "private deletion marker" }),
    ),
  ];
  assert.equal(
    (
  ...
  assert.equal(
    (await send("DELETE", "/api/v1/projects/prj_test/sessions/sess_test"))
      .statusCode,
    200,
  );
  assert.equal(
    JSON.stringify(await db.query("SELECT data FROM orbit_changes")).includes(
      "private deletion marker",
    ),
    false,
  );
  const { repo } = await local(t);
  ...
  await repo.remove("prj_test", "session", "sess_test");
  assert.equal(
    JSON.stringify(await repo.pending("prj_test")).includes(
      "private deletion marker",
    ),
    false,
  );
...
test("recovery drains complete records and retains exclusion state across crashes", async (t) => {
  const { recoverCapture } = await import("../apps/cli/dist/runtime.js");
  const { repo } = await local(t);
